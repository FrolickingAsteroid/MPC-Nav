...
class Simulation:
    def __init__(self, window_size=(800, 600)):
        ...
        # Initialize the control object at the center of the screen
    ...
    def run(self):
        while self.running:
            ...
            self.control_object.update(dt)

            target = self.control_object.get_target()
            state = self.state

            if self.mpc.solve_uav_to_ugv(self.state, target):
                self.state = self.fs.simulate_step(dt=self.mpc.dt, state=self.state,
                    control=self.mpc.get_control(i=0), configs=self.mpc.configs)
                self.path.append(self.state)

            # Update visualization
            self.visualizer.update(self.control_object.get_position(), self.control_object.get_history(), self.state, self.mpc)
