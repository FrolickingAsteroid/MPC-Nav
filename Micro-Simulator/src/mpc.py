...
from Fuzzy import FuzzyBehaviorWeights
...
class MPC(object):
    def __init__(self, T, N, state, map):
        ...
        self.fuzzy = FuzzyBehaviorWeights()
        self.sol = None
    ...
    def mpc_opt(self, state, gp):
        ...
        angle_error = atan2(sin(target_heading - state['theta']), cos(target_heading - state['theta'])) - np.pi / 2
        print(angle_error)
        # Define the cost function
        objective = 0
        objective = self.static_constraint

        weights = self.fuzzy.evaluate(distance_px=sqrt(self.current_distance),
                                    angle_rad=angle_error,
                                    is_moving=target_moving)
        print(weights)
        for k in range(self.N + 1):
            # Apply fuzzy weights to cost terms
            ...
            objective += weights["pursuit"] * ((self.pos_x[k] - gp[0])**2 + (self.pos_y[k] - gp[1])**2)
